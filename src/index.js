...
projectManager.projectList[projIndex].createTask('Buy Groceries', 'Pick up groceries from the store.', '02/10/23', 6);
projectManager.projectList[projIndex].createTask('Wash Dishes', 'Wash today\'s dishes.', '01/02/23', 4);
projectManager.projectList[projIndex].createTask('Walk Dog', 'Walk the dog for 15 minutes.', null, 7);
projectManager.createProject('Monthly Tasks');
projIndex = projectManager.projectList.length - 1;
projectManager.projectList[projIndex].createTask('Pay Bills', 'Pay credit card and electric bills.', null, 10);
projectManager.projectList[projIndex].createTask('Change Bulbs', 'Change light bulbs that are fused.',null, 5);
for (let i=0; i<40; i++) {
}
...
console.log('all tasks: ', projectManager.getAllTasks());
